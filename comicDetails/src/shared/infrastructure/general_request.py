...
class GeneralRequestServer:
    ...
    async def put(self, url: str, body: dict = {}, query_params: dict = {}, headers: dict = {}) -> dict:
        return await self._upsert(url, body, query_params, headers, 'PUT')

    async def delete(self, url: str, query_params: dict = {}, headers: dict = {}) -> dict:
        self.init_time = time.perf_counter()
        headers = {**headers, **self.headers}
        try:
            async with await self._session.client() as session:
                async with session.delete(url, params=query_params, headers=headers) as response:
                    return await self._obtain_response(response, 'DELETE', query_params)
        except Exception as error:
            return self._catched_error(error, url, 'DELETE', query_params)
